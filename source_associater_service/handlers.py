...
def get_ip_data_list(asns):
    ...
    # TODO: confirm that I don't have to unwind
    #unwind = {'$unwind': '$source'}
    group = {
        '$group': {
            '_id': '$ip',
            'sources': {'$push': '$source.name'},
            'ASNumber': {'$first': '$ASNumber'}
        }
    }
    second_project = {
        '$project': {
    ...
    aggregation_pipeline = [project, as_number_match, group, second_project]
...
def add_source_to_ip(ip_data, source_name):
    ...
    if source:
        ...
        # Don't update IP unless it has been analyzed to avoid conflicts with data analytics service while saving.
